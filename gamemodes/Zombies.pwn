...
Hecho en M�xico, con Amors <3 

*/
//==============================================================================
// Includes.
//==============================================================================
//Desmarca este pragma para hacer que el servidor ocupe m�s memoria, en caso de que no quiera iniciar.
...
#define IsNull(%1) \
    ((!(%1[0])) || (((%1[0]) == '\1') && (!(%1[1]))))
#define Maximo_Contrase�as_Incorrectas 3
...
new InfectedZone[MAX_INFECTED_ZONES][InfectedZoneInfo];//INCREMENTAR SI PONGO M�S ZONAS XD LUEGO.
...
//Calcular un circulo alrededor de las coordenadas X,Y, y de ese circulo obtener una posici�n basado en el angulo y el offset con cierta distancia.
...
public OnPlayerPause(playerid)
{
	...
	{
	    printf("[SOBEIT]: %s ha sido expulsado por pausarse en la verificaci�n.",PlayerName2(playerid));
...
public RZClient_OnClientDisconnect(playerid,version[])
{
	...
	SendClientMessage(playerid,red,">] RZCLIENT [< {ffffff}La conexi�n con el servidor se ha perdido.");
	SendClientMessage(playerid,red,"********");
...
PUBLIC:BackupServer()
{
	...
	SendClientMessageToAll(COLOR_AMARILLO,">] RESET [< {ffffff}La copia de seguridad se est� efectuando.");
...
{
    SendClientMessageToAll(red,"********");
	SendClientMessageToAll(red,">] RESET [< {ffffff}Una copia de seguridad ser� efectuada en {FF33CC}5{ffffff} minutos.");
...
public OnPlayerRequestSpawn(playerid)
{
	if(!IsPlayerNPC(playerid))
	{
		...
		{
				new str_DL[256];
				format(str_DL,sizeof(str_DL),"{FFCC33}%s  \n {FFFFFF} Necesitas registrarte, ingresa una contrase�a que recuerdes para continuar.",PlayerName2(playerid));
		...
		{
				new str_DL[256];
				format(str_DL,sizeof(str_DL),"{FFCC33}%s \n {FFFFFF}Bienvenido de vuelta, ingresa tu contrase�a para continuar.",PlayerName2(playerid));
...
stock CheckUserBanned(playerid)
{
	     	...
	     	if(db_num_rows(NUM_ROWS_SQL) != 0)
		    {
				...
				{
				new string[256];
				format(string,sizeof(string), "**{FFFFFF} *%s {4DFFFF} [E]xpulso a {FFFFFF}%s [%d] {BBC1C1}[Raz�n: %s]",bAdmin[playerid],bNick[playerid],playerid,bRazon[playerid]);
				...
				strcat(bDialogSW,bDialog);
				format(bDialog,sizeof(bDialog),"{FFCC33}Raz�n:{FFFFFF} %s\n",bRazon[playerid]);
				strcat(bDialogSW,bDialog);
...
stock CheckRZClientRestricted(playerid)
{
	...
	{
			new string[256];
			format(string,sizeof(string), "**{FFFFFF} %s {4DFFFF} [E]xpulso al jugador sin RZClient {FFFFFF}%s [%d] {BBC1C1}[Raz�n: %s]",bAdmin[playerid],bNick[playerid],playerid,bRazon[playerid]);
			AdminMessageToAdmins(playerid,0x4DFFFFAA,string);
...
stock RCM_ProcessCountry(playerid)
{
	...
	{
		new RZSTR[320];
		format(RZSTR,sizeof(RZSTR),"[ RZ ] {ffffff}%s [%d] Entro a la Revoluci�n de los Zombies [%s].", RCM_PlayerName(playerid), playerid, GetPlayerCountry(playerid));
...
stock IsCountryBanned(playerid,pais[])
{
	...
	{
		new RZSTR[256];
		format(RZSTR,sizeof(RZSTR),"[ RZ ] {ffffff}%s [%d] Fue Expulsado del servidor (%s) Pa�s NO PERMITIDO.", RCM_PlayerName(playerid), playerid,pais);
   		SendClientMessageToAll(COLOR_BLUE_SOLID,RZSTR);
			...
			if(NPC_INFO[npcid][BType] == GetMissionObjType(killerid) && GetMissionType(killerid) == MISSION_KILL_ZOMBIES)
			{
				if(GetMissionProgressAmount(killerid) >= GetMissionProgressLimit(killerid))
				{
					PlayerPlaySound(killerid, 1057, 0.0, 0.0, 0.0);
     				SendClientMessage(killerid,red,"]<!>[ {ffffff}T� mision est� completa, dirigete con cualquier cientifico para recibir tu recompensa.");
				}else{
...
if(!IsPlayerNPC(playerid))
{
	...
	if(PlayerInfo[playerid][MusicInicio] == 1)
	{
		...
		//StopAudioStreamForPlayerEx(playerid);
		//ShowPlayerDialog(playerid, 0, DIALOG_STYLE_MSGBOX, "{00FFFF}RZUpdate: {FF0099}SafeZone", "{FFFFFF}Actualizaci�n {FF0099}SafeZone\n{ffffff}- Iconos de las zonas seguras actualizados!\n-Corregidos bugs de los cientificos.\n-Mortalidad de Cerebro Incrementada!\n-Spawns Humanos actualizados.\n-Nuevo n�cleo n�mero 6." , "OK", "");
		//ShowPlayerDialog(playerid, 0, DIALOG_STYLE_MSGBOX, "{00FFFF}RZUpdate: {FF0099}Drone Beta", "{FFFFFF}Actualizaci�n {FF0099}Drone Beta\n{ffffff}- Se a�adieron drones inteligentes anti zombies." , "OK", "");
		ShowPlayerDialog(playerid, 0, DIALOG_STYLE_MSGBOX, "{FFFF00}Server update: {FF0000}Infected Zone", "{FFFFFF}Actualizaci�n {FF0000}InfectedZone\n{FF00FF}- Zonas de Extrema infecci�n a�adidas!\n{ffffff}- Nuevo NPCZombie Disponible: {{FF00FF}}Heart.\n{ffffff}- Modo cinem�tica a�adido.\n- Spawns Humanos actualizados.\n- Correcci�n de bugs y optimizaci�n interna." , "OK", "");
	...
	{
		new string[256];
		format(string,sizeof(string), "**{FFFFFF} Anti Cheat {4DFFFF}[B]loqueo a {FFFFFF}%s {BBC1C1}[Raz�n: mod Sobeit ]",PlayerName2(playerid));
...
public OnPlayerVehicleHealthHack(playerid)
{
	...
	new string[256];
	format(string,sizeof(string), "**{FFFFFF} Anti Cheat {4DFFFF}Expulso a {FFFFFF}%s {BBC1C1}[Raz�n: Ilegal Vehicle Repair]",PlayerName2(playerid));
	SendClientMessageToAll(0x04DFFFFFF, string);
	Kick(playerid);
...
stock OnPlayerSpawnOK(playerid)
{
	...
	if(!RZClient_IsClientConnected(playerid) && PlayerInfo[playerid][MusicInicio] == 0 && !IsPlayerAdmin(playerid)){
			SendFormatDialog(playerid,"{ffffff}<-- {f50000} RZClient.exe {ffffff}-->","{4DFFFF}Hey {ffffff}%s!\nPara Conseguir m�s beneficios en Nuestro servidor, descarga el {4DFFFF}RZ Client{ffffff} en:\n{4DFFFF}http://bit.ly/RZCLIENTE\n{ffffff}Al descargarlo tambien eliminaras este molesto aviso & conseguiras m�s puntaje & dinero.",PlayerName2(playerid));
...
stock GivePlayerWeaponEx(playerid,weaponid,ammo, bool:droppable = true)
{
	...
	else //Si no tiene munici�n o no se puede tirar, setear la munici�n.
	{
...
public OnPlayerWeaponShot(playerid, weaponid, hittype, hitid, Float: fX,Float: fY,Float: fZ)
{
    ...
    if(!IsPlayerNPC(playerid) && !IsPlayerKicked(playerid))
    {
		...
		if(PlayerInfo[playerid][AAH_WeaponAmmo][stored_weaponid_slot] >= 1)
		{
			...
			if(ilegal_bullets >= 1)// se aumento la munici�n
			{
				new PlayerIP[128];
				GetPlayerIp(playerid,PlayerIP,sizeof(PlayerIP));
		...
		{
		    //guardar en la memoria la cantidad de munici�n del jugador si ya pasaron 500 milisegundos.
...
public OnPlayerDeath(playerid, killerid, reason)
{
    ...
    switch(time - PlayerInfo[playerid][LastDeath]){
        case 0 .. 3:
        {
			...
			format(string, sizeof(string), "{FFFFFF}El Servidor {4DFFFF} [B]loqueo del servidor al Jugador {ffffff}%s. {4DFFFF}[Raz�n: FakeKills ]",PlayerName2(playerid));
...
//	dcmd(me,2,cmdtext);
	...
	SendClientMessage(playerid, COLOR_DE_ERROR, "[ RZ � ]{FFFFFF} /clan Salir{"COLOR_DE_ERROR_EX"} Si usas este comando y eres due�o de un clan este sera eliminado.");
	...
	else if (!strcmp(tmp, "ver", true))
	{
    	...
    	ShowPlayerDialog(playerid,CLAN_VER,DIALOG_STYLE_INPUT,"{ffffff}Ver Informaci�n sobre un CLAN","{ffffff}Por favor ingresa el TAG del clan que deseas ver.",">>","x");
	}
	else{
	SendClientMessage(playerid, COLOR_DE_ERROR, "[ RZ � ]{FFFFFF} /clan Crear{"COLOR_DE_ERROR_EX"} Crea un Clan :D");
	SendClientMessage(playerid, COLOR_DE_ERROR, "[ RZ � ]{FFFFFF} /clan ver{"COLOR_DE_ERROR_EX"} Lista completa de los integrantes de un clan y fundadores.");
...
/*
	...
	if(strcmp("/join", cmdtext, true, 10) == 0)
	{
		...
		if(HumanCore[STATE] == CORE_WAITING || HumanCore[STATE] == CORE_FIGHTING)
		{
		    ...
		    if(CurrentDefenders >= HumanCore[MAX_DEFENDERS] && !IsPlayerAdmin(playerid)) return SendClientMessage(playerid, red, ">> Ya no se admiten m�s reclutas, espera para la pr�xima ronda.");
		...
		{
		     return SendClientMessage(playerid, red, ">> error {ffffff}La misi�n para proteger al nucleo ha comenzado.");
		}*/
	...
	strcat(BIG_DIALOG, ""COL_CMDS"2.\t/mission\t"COL_CMDS"Muestra informaci�n de tu misi�n actual.\n", sizeof(BIG_DIALOG));
...
//	strcat(BIG_DIALOG, ""COLOR_CMDS". "COL_WHITE"\n", sizeof(BIG_DIALOG));
	ShowPlayerDialog(playerid, DIALOG_COMANDOS, DIALOG_STYLE_MSGBOX, ""COL_GB_LIGHT"* [RZ] Comandos: "COL_CMDS"Comando"COL_GB_LIGHT" - "COL_WHITE"Funci�n.", BIG_DIALOG, "More", "Cancel");
	return 1;
	}
	if(strcmp("/Comandos2", cmdtext, true, 10) == 0 || strcmp("/cmds2", cmdtext, true, 10) == 0)
	{
		...
		strcat(BIG_DIALOG, ""COL_CMDS"20.\t/stop\t\t"COL_CMDS"Detiene el AudioStream en reproducci�n.\n", sizeof(BIG_DIALOG));
		ShowPlayerDialog(playerid, 0, DIALOG_STYLE_MSGBOX, ""COL_GB_LIGHT"* [RZ] Comandos: "COL_CMDS"Comando"COL_GB_LIGHT" - "COL_WHITE"Funci�n.", BIG_DIALOG, "OK", "Cancel");
		return 1;
	}
	...
	{
	//ShowPlayerDialog(playerid, 0, DIALOG_STYLE_MSGBOX, "{FFFF00}Reclamaci�n de VIP (RVIP).", "Este comando fue desactivado, para obtener vip debes pedirlo a x[R]icard[O.O]x", "x", "");
	...
	if(GetPlayerScore(playerid) >= VIP_SCORE_SILVER && GetPlayerScore(playerid) < VIP_SCORE_GOLD && PlayerInfo[playerid][VIP] < 1)
	{
		...
		ShowPlayerDialog(playerid, 0, DIALOG_STYLE_MSGBOX, "{FFFF00}Reclamaci�n de VIP (RVIP).", rVIP, "x", "");
	...
	{
       if(!IsPlayerAdmin(playerid)) return SendClientMessage(playerid,0xCC0000FF, "�o :3");
	...
	|| strcmp(cmdtext, "/drop", true) == 0)
    {
        ...
        if(GunID > 0 && GunAmmo != 0)
        {
            ...
            if(f > MAX_DROPPED_WEAPONS) return SendClientMessage(playerid, COLOR_DE_ERROR, "Es imposible soltar el arma en este momento, intentalo m�s tarde!");
	...
	strcat(CMDS, "{9C00EB} 6. {FFFFFF}ESTA ESTRICTAMENTE PROH�BIDO SER COMPLICE DE LOS HACKERS {9C00EB}(UBAN)\n", sizeof(CMDS));
	strcat(CMDS, "{9C00EB} 7. {FFFFFF}NO SE PUEDE PEDIR ADMIN, CUANDO HAYA VACANTES SER� TU TURNO {9C00EB}(CALLAR)\n", sizeof(CMDS));
	...
	strcat(CMDS, "{9C00EB} 11. {FFFFFF}ESTA ESTRICTAMENTE PROHIBIDO REGALAR MUERTES! (SCORE F�CIL){9C00EB}(UBAN)\n", sizeof(CMDS));
	strcat(CMDS, "{9C00EB} 12. {FFFFFF}CONTINUACI�N ==> /rules2\n", sizeof(CMDS));
	...
	{
	new CMDS[325];
	strcat(CMDS, "{9C00EB} 12. {FFFFFF}QUEDA ESTRICTAMENTE PROHIBIDO PERTURBAR LA PAZ P�BLICA DEL SERVER{9C00EB}(CALLAR)\n", sizeof(CMDS));
	...
	strcat(CMDS, "{9C00EB} 3. {FFFFFF}SI UN USUARIO SOSPECHAS TIENE HACK, NO PUEDES BANEARLO HASTA QUE MATE A ALGUIEN M�S\n", 1024);
	strcat(CMDS, "{9C00EB} 3. {FFFFFF}SI LO BANEAS Y NO TIENES PRUEBAS EXACTAS DEL HACK O NO FUE ACUSADO POR ALGUIEN M�S\n", 1024);
	strcat(CMDS, "{9C00EB} 3. {FFFFFF}SER�S EXPULSADO DE LA ADMINISTRACI�N DEBIDO A LA REGLA 12 DE LOS USUARIOS.\n", 1024);
	...
	strcat(CMDS, "{9C00EB} 8. {FFFFFF}CUALQUIER BANEO DEBERA CONTENER EN LA RAZ�N UNA V�LIDA Y CORRECTA DEL MISMO.\n", 1024);
	...
	strcat(CMDS, "{9C00EB} 10. {FFFFFF}ESTRA ESTRICTAMENTE PROHIBIDO EXPULSAR, EXPLOTAR, BANEAR O CALLAR SIN RAZONES V�LIDAS!\n", 1024);
	...
	strcat(CMDS, "{9C00EB} 14. {FF0000}ESTA ESTRICTAMENTE PROHIBIDO UTILIZAR LSAY COMO CHAT COM�N!{9C00EB}(CALLAR/-1LVL)\n", 1024);
	ShowPlayerDialog(playerid, 0, DIALOG_STYLE_MSGBOX, " {00FF19}RZ: {FFFFFF}2016 ADMIN RULES ", CMDS, "OK", "") ;
	return 1;
	}
	...
	{
	  	if(CheckAdminsOnline() < 2) return SendClientMessage(playerid, COLOR_DE_ERROR, "[� ERROR �]: {FFFFFF}No hay m�s de 2 administradores conectados, Por lo tanto, no puedes enviar dinero.");
		if(PlayerInfo[playerid][Identificado] == 0 || PlayerInfo[playerid][RZClient_Verified] == 0){SendClientMessage(playerid, COLOR_DE_ERROR, "[ ERROR ]{ffffff} Para utilizar el chat necesitas identificarte/spawnear por primera vez."); return 0;}
        ...
        if (IsPlayerConnected(giveplayerid))
		{
			...
			{
	        	SendClientMessage(playerid, COLOR_DE_ERROR, "[� ERROR �]: {FFFFFF}acci�n no permitida.");
	        }
        }
		else
		{
	...
	if(strcmp("/cancion",cmdtext, true) == 0)
	{
		...
		{
			ShowPlayerDialog(playerid,NMusica2,DIALOG_STYLE_INPUT,"NLink","{ffffff}Pon el Link de La Canci�n\n\nque no sea demasiado largo o podria crashear el servidor.","Seguir","x");
		}
		else
		{
    ...
    {
	ShowPlayerDialog(playerid,SongDialog,DIALOG_STYLE_INPUT,"{ffffff}Buscador de Musica {f50000}=D","{ffffff}Escribe el nombre de la canci�n / sonido o parte del nombre para escucharla\n\n{f50000}* si deseas introducir tu propio enlace utiliza /link [enlace]","Play","x");
    return 1;
    }
	...
	ShowPlayerDialog(playerid,SongDialogADM,DIALOG_STYLE_INPUT,"{ffffff}Buscador de Musica {f50000}=D","{ffffff}Escribe el nombre de la canci�n / sonido o parte del nombre para Reproducirla a Todos\n\n{f50000}* si deseas introducir tu propio enlace utiliza /cancion","Play","x");
    PlayerPlaySound(playerid,1057,0.0,0.0,0.0);
	return 1;
	}
    ...
    strcat(CMDS,"{FF0000}Due�o: {FFFFFF} es.zRicardo\n", sizeof(CMDS));
    ...
    format(string1,sizeof(string1), "%s Te Ha otorgado una cuenta En La Administraci�n de Nivel: %d. Usa /Acmds y /Acmds2 | Si abusas Puedes ser DesPromovido.", nombreusador,strval(tmp2));
...
//------------------------------------------------------------------------------
	...
	if (PlayerInfo[playerid][VIP] >= 1 )
	{
	if(GetPlayerState(playerid) == PLAYER_STATE_SPECTATING && !IsPlayerAdmin(playerid)) return SendClientMessage(playerid, COLOR_ROJO, "Error >> {FFFFFF}Despidete de tu cuenta, ruf�an.");
	...
	new CMDERROR[256];
	format(CMDERROR,sizeof(CMDERROR),"[� ERROR �]:{ffffff} El Comando {"COLOR_DE_ERROR_EX"}%s{ffffff} no existe, utiliza /cmds para ver m�s comandos.",cmdtext);
...
if(!strlen(tmp2)) return SendClientMessage(playerid, red, "Uso correcto: /cambiarpass [Nueva contrase�a]");
if(strlen(tmp2) > 14) return SendClientMessage(playerid, red, "ERROR: La contrase�a contiene mas de 14 caracteres!.");
if(strlen(tmp2) < 3) return SendClientMessage(playerid, red, "ERROR: La contrase�a debe ser mayor a 3 caracteres.");
...
SendClientMessage(playerid,green,"Tu contrase�a fue cambiada satisfactoriamente, este cambio NO afectara tu cuenta del foro.");
}
return 1;
}
...
dcmd_kick(playerid, params[]){
  if(PlayerInfo[playerid][nivel] >= 1 || IsPlayerAdmin(playerid)){
    ...
    if(!strlen(params) || !IsNumeric(tmp))return SendClientMessage(playerid, COLOR_BLANCO, "USO: /kick <playerid> <raz�n>");
    ...
    if(IsPlayerConnected(jugador) || jugador != INVALID_PLAYER_ID){
      if(PlayerInfo[jugador][nivel] < PlayerInfo[playerid][nivel] || !IsPlayerAdmin(jugador)){
        ...
        format(str, sizeof(str), "[KICK] Usuario %s Expulsado Por %s Raz�n: %s",name, admin, tmp2);
...
dcmd_dbug(playerid, params[]){
  if(PlayerInfo[playerid][nivel] >= 1 || IsPlayerAdmin(playerid)){
    ...
    tmp = strtok(params, Index);
    tmp2 = strrest(params, Index);
    if(!strlen(params) || !IsNumeric(tmp))return SendClientMessage(playerid, COLOR_BLANCO, "USO: /dbug <playerid> <raz�n> | reconecta a un jugador, adecuado para desbugear usuarios.");
    new jugador, name[MAX_PLAYER_NAME], admin[MAX_PLAYER_NAME], str[300];
    jugador = strval(tmp);
    if(!IsPlayerConnected(jugador)) return SendClientMessage(playerid, COLOR_BLANCO, "Jugador no conectado.");
    if (IsPlayerNPC(jugador)) return SendClientMessage(playerid,red, "ERROR: No expulses a los bots.");
    if(IsPlayerConnected(jugador) || jugador != INVALID_PLAYER_ID){
      if(PlayerInfo[jugador][nivel] < PlayerInfo[playerid][nivel] || !IsPlayerAdmin(jugador)){
        ...
        format(str, sizeof(str), "[RE-CONNECT] Usuario %s Reconectado Por %s Raz�n: %s",name, admin, tmp2);
        SaveCMD(playerid, str);
...
dcmd_chbug(playerid, params[]){
  if(PlayerInfo[playerid][nivel] >= 1 || IsPlayerAdmin(playerid)){
    new tmp[256], Index;
    tmp = strtok(params, Index);
    if(!strlen(params) || !IsNumeric(tmp))return SendClientMessage(playerid, COLOR_BLANCO, "USO: /chbug <playerid> <raz�n> | verifica si un jugador esta bug (esta funcion es inestable).");
    new jugador,str[300];
    jugador = strval(tmp);
    if(PlayerInfo[jugador][nivel] < PlayerInfo[playerid][nivel] || !IsPlayerAdmin(jugador))
	{
...
dcmd_sbcheck(playerid, params[]){
  if(PlayerInfo[playerid][nivel] >= 1 || IsPlayerAdmin(playerid))
  {
    ...
    if(PlayerInfo[jugador][nivel] < PlayerInfo[playerid][nivel] || !IsPlayerAdmin(jugador))
	{
		...
		return SendClientMessage(playerid, COLOR_ROJO, "Success >> {FFFFFF}El servidor se encargara de todo, no es necesario hacer nada m�s.");
  ...
  if(PlayerInfo[playerid][nivel] >= 1 || IsPlayerAdmin(playerid)){
    ...
    if(IsPlayerConnected(jugador) || jugador != INVALID_PLAYER_ID){
      if(PlayerInfo[jugador][nivel] < PlayerInfo[playerid][nivel] || !IsPlayerAdmin(jugador)){
        ...
        printf("[KICK] Usuario %s Expulsado Por %s Raz�n: %s [%d/%d]",admin, name, tmp2,PlayerInfo[jugador][Advertencias],ADVERTENCIAS_MAXIMAS);
        PlayerMessageToPlayers(playerid, -1, str);
		return Kick(jugador);
		}
		...
		printf("[WARN] Usuario %s Advertido Por %s Raz�n: %s",admin, name, tmp2);
		return 1;
      }else return SendClientMessage(playerid, COLOR_ROJO, "ERROR: No puedes usar este comando sobre este jugador");
    }else return SendClientMessage(playerid, COLOR_ROJO, "ERROR: El jugador no esta conectado o la id no es correcta");
  }else return SendClientMessage(playerid, COLOR_ROJO, "ERROR: No posees el nivel suficiente para usar este comando");
...
dcmd_votekick(playerid, params[])
{
  ...
  //{
    ...
    tmp = strtok(params, Index);
    tmp2 = strrest(params, Index);
    if(!strlen(params) || !IsNumeric(tmp))return SendClientMessage(playerid, COLOR_BLANCO, "USO: /VoteKick <playerid> <raz�n>");
    jugador = strval(tmp);
	...
	if (Voto[playerid] == 1) return SendClientMessage(playerid,red, "ERROR: Ya has Votado, Espera a que voten m�s personas para votar.");
    if(!IsNull(tmp2) && Votos_KICK[jugador] == 0 && strlen( RemoveSpaces(tmp2) ) < 3 )return SendClientMessage(playerid,red, "[ RZ; Raz�n Invalida ]: {ffffff}Utiliza este comando responsablemente o te bloquearemos la cuenta.");
    if(IsPlayerConnected(jugador) && jugador != INVALID_PLAYER_ID)
	{
		...
		if(Votos_KICK[jugador] >= VOTOS_PARA_KICK)
		{
			...
			printf("[VoteKick:KICK] Usuario %s Dio El Ultimo Voto para Expulsar a %s Raz�n: %s [%d/%d]",PlayerName2(playerid), PlayerName2(jugador), RAZON_KICK[jugador],PlayerInfo[jugador][Advertencias],ADVERTENCIAS_MAXIMAS);
		...
		Voto[playerid] = 1;
		VoteKickFlood[playerid] = rztickcount();
		printf("[VoteKick:WARN] Usuario %s Advertido Por %s Raz�n: %s",PlayerName2(playerid), PlayerName2(jugador), tmp2);
		return 1;
	}else return SendClientMessage(playerid, COLOR_ROJO, "ERROR: El jugador no esta conectado o la id no es correcta");
...
dcmd_sban(playerid, params[]){
  if(PlayerInfo[playerid][nivel] >= 6 || IsPlayerAdmin(playerid)){//restriccion para que solo los admins de nivel 4 o superior puedan usar el comando
    ...
    if(!strlen(params))return SendClientMessage(playerid, COLOR_BLANCO, "USO: /sban <nombre> <raz�n> | bloquea la cuenta de un usuario");//comprobamos que hayan igresado los argumentos, y que la id sea un numero.
    ...
    if(!IsNull(name))
    {
        ...
        AdminMessageToAdmins(playerid, COLOR_GRIS, str);
        format(str, sizeof(str), "%s Baneado por el admin %s, [Raz�n: %s] \n",name, admin, tmp2);
        SaveCMD(playerid, str);
...
dcmd_rzcban(playerid, params[]){
  if(PlayerInfo[playerid][nivel] >= 6 || IsPlayerAdmin(playerid)){//restriccion para que solo los admins de nivel 4 o superior puedan usar el comando
    ...
    name = strtok(params, Index);//tomara el 1 valor del comando, es decir la id del jugador
    tmp2 = strrest(params, Index);//tomara el 2 valor del comando, es decir la razon del ban
    if(!strlen(params))return SendClientMessage(playerid, COLOR_BLANCO, "USO: /rzcban <nombre> <raz�n> | bloquea la cuenta de un usuario solo para RZC");//comprobamos que hayan igresado los argumentos, y que la id sea un numero.
    if(!strlen(tmp2))return SendClientMessage(playerid, COLOR_ROJO, "ERROR: ingresa una razon y asegurate de que sea valida.");
    new admin[MAX_PLAYER_NAME], str[256];
    if(!IsNull(name))
    {
        ...
        AdminMessageToAdmins(playerid, COLOR_GRIS, str);
        format(str, sizeof(str), "%s RZClient restricted por el admin %s, [Raz�n: %s] \n",name, admin, tmp2);
        SaveCMD(playerid, str);
...
dcmd_cban(playerid, params[]){
  if(PlayerInfo[playerid][nivel] >= 6 || IsPlayerAdmin(playerid)){//restriccion para que solo los admins de nivel 4 o superior puedan usar el comando
    ...
    name = strtok(params, Index);//tomara el 1 valor del comando, es decir la id del jugador
    tmp2 = strrest(params, Index);//tomara el 2 valor del comando, es decir la razon del ban
    if(!strlen(params))return SendClientMessage(playerid, COLOR_BLANCO, "USO: /cban <nombre> <raz�n> | Elimina la cuenta de un usuario");//comprobamos que hayan igresado los argumentos, y que la id sea un numero.
    if(!strlen(tmp2))return SendClientMessage(playerid, COLOR_ROJO, "ERROR: ingresa una razon y asegurate de que sea valida.");
    new admin[MAX_PLAYER_NAME], str[256];
    if(!IsNull(name))
    {
        ...
        AdminMessageToAdmins(playerid, COLOR_GRIS, str);
        format(str, sizeof(str), "%s Eliminado por el admin %s, [Raz�n: %s] \n",name, admin, tmp2);
        SaveCMD(playerid, str);
...
dcmd_ban(playerid, params[]){
  if(PlayerInfo[playerid][nivel] >= 2 || IsPlayerAdmin(playerid)){//restriccion para que solo los admins de nivel 4 o superior puedan usar el comando
    ...
    tmp2 = strrest(params, Index);//tomara el 2 valor del comando, es decir la razon del ban
    if(!strlen(params) || !IsNumeric(tmp))return SendClientMessage(playerid, COLOR_BLANCO, "USO: /ban <playerid> <raz�n>");//comprobamos que hayan igresado los argumentos, y que la id sea un numero.
    if(!strlen(tmp2))return SendClientMessage(playerid, COLOR_ROJO, "ERROR: Debes ingresar una raz�n");
    ...
    if(IsPlayerConnected(jugador) || jugador != INVALID_PLAYER_ID){ //restriccion para saber si el jugador que se quiere banear esta conectado o no, y si la id es valida
      if(PlayerInfo[jugador][nivel] < PlayerInfo[playerid][nivel] || !IsPlayerAdmin(jugador)){ //restriccion para saber si el jugador que se quiere banear posee un nivel de admin mayor o es admin RCON
        ...
        AdminMessageToAdmins(playerid,COLOR_GRIS, str);
        printf("[NOTICIA] Usuario %s Baneado Por %s Raz�n: %s",name, admin, tmp2);
        format(str, sizeof(str), "%s Baneado Por el miembro del staff %s, [Raz�n: %s] \n",name, admin, tmp2);
        SaveCMD(playerid, str);
  ...
  if(PlayerInfo[playerid][nivel] >= 2 || IsPlayerAdmin(playerid)){//restriccion para que solo los admins de nivel 4 o superior puedan usar el comando
    ...
    if(IsPlayerConnected(jugador) || jugador != INVALID_PLAYER_ID){ //restriccion para saber si el jugador que se quiere banear esta conectado o no, y si la id es valida
      if(PlayerInfo[jugador][nivel] < PlayerInfo[playerid][nivel] || !IsPlayerAdmin(jugador)){ //restriccion para saber si el jugador que se quiere banear posee un nivel de admin mayor o es admin RCON
        ...
        AdminMessageToAdmins(playerid,COLOR_GRIS, str);
        printf("[NOTICIA] Usuario %s Restringido Por %s Raz�n: %s",name, admin, tmp2);
        format(str, sizeof(str), "%s Restringido Por el miembro del staff %s, [Raz�n: %s] \n",name, admin, tmp2);
        SaveCMD(playerid, str);
...
dcmd_uban(playerid, params[]){
  if(IsPlayerAdmin(playerid)){
    ...
    tmp = strtok(params, Index);
    tmp2 = strrest(params, Index);
    if(!strlen(params) || !IsNumeric(tmp))return SendClientMessage(playerid, COLOR_BLANCO, "USO: /uban <playerid> <raz�n> | Elimina la cuenta de un usuario de la base de datos.");
    if(!strlen(tmp2))return SendClientMessage(playerid, COLOR_ROJO, "ERROR: Debes ingresar una raz�n");
    new jugador, name[MAX_PLAYER_NAME],admin[MAX_PLAYER_NAME], str[300];
    jugador = strval(tmp);
    if(!IsPlayerConnected(jugador)) return SendClientMessage(playerid, COLOR_BLANCO, "Jugador no conectado.");
    ...
    if(IsPlayerConnected(jugador) || jugador != INVALID_PLAYER_ID){
      if(PlayerInfo[jugador][nivel] < PlayerInfo[playerid][nivel] || !IsPlayerAdmin(jugador)){
        ...
        AdminMessageToAdmins(playerid,COLOR_GRIS, str);
        printf("[U-BAN]: Usuario %s Eliminado de la DB Por %s[%d] Raz�n: %s",name, admin,playerid, tmp2);
        format(str, sizeof(str), "%s Eliminado de la DB Por %s, [Raz�n: %s] \n",name, admin, tmp2);
        SaveCMD(playerid, str);
		new DeleteAccount[100];
		format(DeleteAccount, sizeof(DeleteAccount), "DELETE FROM Usuarios WHERE Username='%s'",name);
		db_query(DataBase_USERS,DeleteAccount);
...
dcmd_callar(playerid, params[]){
  if(PlayerInfo[playerid][nivel] >= 1 || IsPlayerAdmin(playerid)){
    ...
    tmp = strtok(params, Index);
    tmp2 = strrest(params, Index);
    if(!strlen(params) || !IsNumeric(tmp))return SendClientMessage(playerid, COLOR_BLANCO, "USO: /callar <playerid> opcional <raz�n>");
    new jugador, name[MAX_PLAYER_NAME], admin[MAX_PLAYER_NAME], str[300];
    jugador = strval(tmp);
      ...
      if(PlayerInfo[jugador][nivel] < PlayerInfo[playerid][nivel] || !IsPlayerAdmin(jugador)){
        ...
        if(PlayerInfo[jugador][callado] == 0){
          if(!strlen(tmp2)){
            ...
            format(str, sizeof(str), "{4DFFFF}El Administrador {ffffff}%s{4DFFFF} ha callado a {ffffff}%s. {4DFFFF}[Raz�n: %s]", admin, name, tmp2);
          }
          AdminMessageToAdmins(playerid,COLOR_GRIS, str);
		...
		if(PlayerInfo[playerid][nivel] >= 5) {
			...
			if(PlayerInfo[player1][nivel] >= 3 && !IsPlayerAdmin(playerid)) return SendClientMessage(playerid,red,"ERROR: No puedes Banear a este Admin Por que no Estas Autorizado Para Banear Admins xD.");
		 	if(IsPlayerConnected(player1) && player1 != INVALID_PLAYER_ID && player1 != playerid) {
            	...
            	printf("[NOTICIA] Usuario %s [R]Baneado Por %s Raz�n: %s",playername,adminname,tmp2);
...
dcmd_gcardb(playerid,params[]) {
        ...
        SaveSQLVehicle(vID);
		SendClientMessage(playerid,COLOR_ARTICULO,"Success: El vehiculo ha sido insertado en la base de datos y estar� listo en el proximo reinicio.");
        return 1;
}
...
dcmd_setzspawntime(playerid,params[]) {
	if(IsPlayerAdmin(playerid)) {
		...
		new var = strval(params);
		if(var > 600 && var <= 0) return SendClientMessage(playerid, red, "No se permite menos de 1 segundo o m�s de 600 segundos (10 minutos).");
...
dcmd_setzspawnrange(playerid,params[]) {
	if(IsPlayerAdmin(playerid)) {
		...
		if(var >= ZombieVisionDistance) return SendFormatMessage(playerid, red, "No se permite una distancia de spawn mayor o igual que la visi�n del zombie(%.02f).", ZombieVisionDistance);
...
dcmd_setzvision(playerid,params[]) {
	if(IsPlayerAdmin(playerid)) {
		...
		new string[256];
		format(string,sizeof(string),"* %s{4DFFFF} cambio la capacidad de visi�n de los zombies, {ffffff}%.02f metros.", pName(playerid), ZombieVisionDistance);
		return PlayerMessageToPlayers(playerid, -1, string);
	} else return SendClientMessage(playerid,red,"ERROR: You are not a high enough level to use this command");
}
...
dcmd_congelar(playerid, params[]){
  if(PlayerInfo[playerid][nivel] >= 1 || IsPlayerAdmin(playerid)){
    ...
    tmp = strtok(params, Index);
    tmp2 = strtok(params, Index);
    if(!strlen(params) || !IsNumeric(tmp))return SendClientMessage(playerid, COLOR_BLANCO, "USO: /congelar <playerid> opcional <raz�n>");
    ...
    if(IsPlayerConnected(jugador) || jugador != INVALID_PLAYER_ID){
      if(PlayerInfo[jugador][nivel] < PlayerInfo[playerid][nivel] || !IsPlayerAdmin(jugador)){
        ...
        if(PlayerInfo[jugador][congelado] == 0){
          ...
          printf("[KICK] Usuario %s Congelado Por %s Raz�n: %s",admin, name, params[2]);
          ...
          printf("[KICK] Usuario %s DesCongelado Por %s Raz�n: %s",admin, name, params[2]);
		...
		if(!IsPlayerAdmin(playerid) && IsPlayerAdmin(player1)) return SendClientMessage(playerid, COLOR_DE_ERROR, "ERROR: NO PUEDES UTILIZAR ESTE COMANDO EN EL DIOS DEL SERVER XD.");
	 	if(IsPlayerConnected(player1) && player1 != INVALID_PLAYER_ID && player1 != playerid) {
			...
			if(PlayerInfo[playerid][Ocultado] == 0 && !IsPlayerAdmin(player1)){
				format(string,sizeof(string),"El Administrador %s [ID: %d] se reubicado a tu posici�n actual.", PlayerName2(playerid), playerid);
				SendClientMessage(player1,blue,string);
			}
			format(string,sizeof(string),"Posici�n cambiada hacia las coordenadas de \"%s\"", pName(player1));
			return SendClientMessage(playerid,blue,string);
...
dcmd_explode(playerid,params[]) {
		if(PlayerInfo[playerid][nivel] >= 1) {
			...
			player1 = strval(tmp);
			if(!strlen(razon)) return SendClientMessage(playerid, red, "Tienes que poner una raz�n.");
			if(!IsPlayerAdmin(playerid) && IsPlayerAdmin(player1))
			{
			...
			return CreateExplosionForPlayer(playerid,burnx, burny , burnz, 7,30.0);
		 	...
		 	if(IsPlayerConnected(player1) && player1 != INVALID_PLAYER_ID ) {
				...
				format(str, sizeof(str), "* Admin %s Exploto al usuario %s, Raz�n: %s", adminname,playername,razon);
				printf(str);
				//SaveCMD(playerid, str);
...
if(SinCHAT == 1&& !IsPlayerAdmin(playerid)) return SendClientMessage(playerid,COLOR_DE_ERROR, "[ ERROR ] {FFFFFF}El chat esta desactivado."), 0;
	...
	reported = strval(tmp);
	if(IsPlayerNPC(reported))ShowPlayerDialog(playerid, 0, DIALOG_STYLE_MSGBOX, "{F50000}Retardado", "TU REPORTE NO FUE ENVIADO, HAS REPORTADO UN ZOMBIE BOT, VERIFICA LOS ESPACIOS EN T� COMANDO.", "x", "");
 	if(IsPlayerConnected(reported) && reported != INVALID_PLAYER_ID) {
		...
		if(strlen(tmp2) >= 55) return SendClientMessage(playerid,red,"ERROR: El Reporte tiene mas de 50 caracteres hazlo mas corto.");
			...
			printf("[/REPORTAR] %s ha reportado al jugador %s, Raz�n: %s",reporter,reportedname,params[strlen(tmp)+1]);
	...
	if(IsPlayerNPC(reported))ShowPlayerDialog(playerid, 0, DIALOG_STYLE_MSGBOX, "{F50000}Retardado", "TU REPORTE NO FUE ENVIADO, HAS REPORTADO UN ZOMBIE BOT, VERIFICA LOS ESPACIOS EN T� COMANDO.", "x", "");
 	if(IsPlayerConnected(reported) && reported != INVALID_PLAYER_ID) {
		...
		if(strlen(tmp2) >= 55) return SendClientMessage(playerid,red,"ERROR: The report is long, please cut the text.");
			...
			for(new i = 1; i < MAX_REPORTS-1; i++) Reports[i] = Reports[i+1];
			Reports[MAX_REPORTS-1] = str;
			printf("[/REPORT] %s ha reportado al jugador %s, Raz�n: %s",reporter,reportedname,params[strlen(tmp)+1]);
...
dcmd_link(playerid,params[])
{
        ...
        if(strlen(params[0]) >= 256) return SendClientMessage(playerid,ADMIN_COLOR, "El nombre de la canci�n es demaciado largo D:");
		new string[280];
...
{
		if(!strlen(params)) return SendClientMessage(playerid, red, "[-] Error al reproducir la canci�n.");
		new string[450];
		format(string, sizeof(string), "%s",params[0]);
	 	PlayAudioStreamForPlayerEx(playerid,string);
		return SendClientMessage(playerid, -1, "Esta reproduciendo un audio, utilice /stop para detenerlo.");
...
{
	if(PlayerInfo[playerid][nivel] < 2) return ShowPlayerDialog(playerid, 0, DIALOG_STYLE_MSGBOX, "{ffffff}Administraci�n:{f50000} ZR", "{4DFFFF}La Lista de Administraci�n de RZ no esta disponible.", "x", "");
	#pragma unused params
	...
	if(conteo == 0) ShowPlayerDialog(playerid, 0, DIALOG_STYLE_MSGBOX, "{ffffff}Administraci�n:{f50000} ZR", "{4DFFFF}La Lista de Administraci�n de Zombie Revolution No se da a conocer por razones de seguridad.", "x", "");
...
dcmd_peers(playerid,params[])
{
	...
	if(conteo == 0) ShowPlayerDialog(playerid, 0, DIALOG_STYLE_MSGBOX, "{ffffff}RZClient:{f50000} PEERS", "{4DFFFF}Ningun usuario est� conectado con el RZClient.", "x", "");
...
PUBLIC:AssignDroneToPlayer(AssignedDrone, playerid, max_owner_distance, max_attack_distance, min_attack_distance, min_attack, max_attack)
{
	...
	SendFormatMessage(playerid, COLOR_ARTICULO,"]> DroneSystem <[ {FFFF33} %s {ffffff}Rango de seguimiento: %dkm, Rango de detecci�n enemiga: %dkm.", PlayerName2(AssignedDrone), max_owner_distance, max_attack_distance);
...
stock NegociarDrone(playerid, name[], price, max_owner_distance, max_attack_distance, min_attack_distance, min_attack, max_attack)
{
    ...
    if(AssignedDrone == INVALID_PLAYER_ID) return SendClientMessage(playerid, COLOR_ARTICULO, "[COMERCIO]: {ffffff} se me han agotado los drones!, vuelve m�s tarde.");
	if(PlayerInfo[playerid][DRONE_ID] != INVALID_PLAYER_ID)
	{
		FreeDroneBOT(PlayerInfo[playerid][DRONE_ID], playerid);
	}
...
stock NegociarAntibiotico(playerid, antibiotic[], precio, Float:salud)
{
	...
	return SendFormatMessage(playerid, COLOR_ARTICULO, "[COMERCIO]: {ffffff} Has adquirido el antibiotico {FF0099}%s{ffffff} Ahora tu salud est� al {FF0099}%.1f {ffffff}porciento.",antibiotic,salud);
}
...
public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
	...
	SendClientMessage(playerid,red,"]<!>[ {ffffff}Diviertete en el modo supervivencia, es cas� �nico.");
	...
	if(response)
	{
		...
		else
		{
			new SString[119];
			PlayerPlaySound(playerid, 1053, 0.0, 0.0, 0.0);
			format(SString,sizeof(SString),"{9C00EB}La Contrase�a: '{ffffff} %s {9C00EB}' que has tipeado es incorrecta, intentalo de nuevo.",inputtext);
...
*/
	...
	if(dialogid == SongDialog)
	{
		if(response)
		{
		if(strlen(inputtext) >= 100) return SendClientMessage(playerid,ADMIN_COLOR, "El nombre de la canci�n es demaciado largo D:");
		StopAudioStreamForPlayerEx(playerid);
...
stock ANNOUNCE_BUG(str_announce[])
{
        ...
        for (iLen = strlen(str_announce); iPos < iLen; iPos ++)
            ...
            {
                case '�':   an_str[iPos] = 151;
                case '�':   an_str[iPos] = 152;
                case '�':   an_str[iPos] = 153;
                case '�':   an_str[iPos] = 154;
                case '�':   an_str[iPos] = 128;
                case '�':   an_str[iPos] = 129;
                case '�':   an_str[iPos] = 130;
                case '�':   an_str[iPos] = 131;
                case '�':   an_str[iPos] = 157;
                case '�':   an_str[iPos] = 158;
                case '�':   an_str[iPos] = 159;
                case '�':   an_str[iPos] = 160;
                case '�':   an_str[iPos] = 134;
                case '�':   an_str[iPos] = 135;
                case '�':   an_str[iPos] = 136;
                case '�':   an_str[iPos] = 137;
                case '�':   an_str[iPos] = 161;
                case '�':   an_str[iPos] = 162;
                case '�':   an_str[iPos] = 163;
                case '�':   an_str[iPos] = 164;
                case '�':   an_str[iPos] = 138;
                case '�':   an_str[iPos] = 139;
                case '�':   an_str[iPos] = 140;
                case '�':   an_str[iPos] = 141;
                case '�':   an_str[iPos] = 165;
                case '�':   an_str[iPos] = 166;
                case '�':   an_str[iPos] = 167;
                case '�':   an_str[iPos] = 168;
                case '�':   an_str[iPos] = 142;
                case '�':   an_str[iPos] = 143;
                case '�':   an_str[iPos] = 144;
                case '�':   an_str[iPos] = 145;
                case '�':   an_str[iPos] = 169;
                case '�':   an_str[iPos] = 170;
                case '�':   an_str[iPos] = 171;
                case '�':   an_str[iPos] = 172;
                case '�':   an_str[iPos] = 146;
                case '�':   an_str[iPos] = 147;
                case '�':   an_str[iPos] = 148;
                case '�':   an_str[iPos] = 149;
                case '�':   an_str[iPos] = 174;
                case '�':   an_str[iPos] = 173;
                case '�':   an_str[iPos] = 64;
...
public OnPlayerHackDetected(playerid, hack_type)
{
	switch(hack_type)
	{
	        case MONEY_HACK:
	        {
				...
				new string[256];
				format(string, sizeof(string), "{FFFFFF}El Servidor {4DFFFF}[B]loqueo del servidor al Jugador {ffffff}%s. {4DFFFF}[Raz�n: Money Hack ]",PlayerName2(playerid));
				SendClientMessageToAll(-1, string);
	        ...
	        case CAR_HACK:
	        {
				...
				new string[256];
				format(string, sizeof(string), "{FFFFFF}El Servidor {4DFFFF}[E]xpulso del servidor al Jugador {ffffff}%s. {4DFFFF}[Raz�n: CLEO HACKING #iTroll ]",PlayerName2(playerid));
				SendClientMessageToAll(-1, string);
	            Kick(playerid);
	            return 0;
	        }
...
/*public OnPlayerCleoDetected( playerid, cleoid )
{
	    switch(cleoid)
	    {
	        case CLEO_CARWARP:
	        {
				if(IsPlayerAdmin(playerid)) return 0;
				new string[256];
				format(string, sizeof(string), "{FFFFFF}El Servidor {4DFFFF} [B]loqueo del servidor al Jugador {ffffff}%s. {4DFFFF}[Raz�n: CLEO HACKING #CW ]",PlayerName2(playerid));
				SendClientMessageToAll(-1, string);
	        ...
	        case CLEO_CAR_PARTICLE_SPAM:
	        {
				if(IsPlayerAdmin(playerid)) return 0;
				new string[256];
				format(string, sizeof(string), "{FFFFFF}El Servidor {4DFFFF} [B]loqueo del servidor al Jugador {ffffff}%s. {4DFFFF}[Raz�n: CLEO HACKING #PS ]",PlayerName2(playerid));
				SendClientMessageToAll(-1, string);
...
public OnVehicleMod(playerid, vehicleid, componentid)
{
	...
	{
	new string[256];
	format(string, sizeof(string), "{FFFFFF}El Servidor {4DFFFF} [B]loqueo del servidor al Jugador {ffffff}%s. {4DFFFF}[Raz�n: CRASHER BITCH �� ]",PlayerName2(playerid));
	SendClientMessageToAll(-1, string);
...
case 2:SendClientMessageToAll(COLOR_ARTICULO, "[+]{FFFFFF} S� tiene alguna duda acerca del servidor, utilice /ask, los moderadores le atenderan. ");
...
case 4:SendClientMessageToAll(COLOR_ARTICULO, "[+]{FFFFFF} �Sab�as Qu�.. Puedes comprar un drone express en cualquier lugar? /buydrone");
case 5:SendClientMessageToAll(COLOR_ARTICULO, "[+]{FFFFFF} �Sab�as qu�.. Es posible adquirir drones m�s baratos en el simbolo de la bandera?");
...
case 8:SendClientMessageToAll(COLOR_ARTICULO, "[+]{FFFFFF} �Aburrido? Descarga el RZClient y m�ximiza tu diversi�n! {FF0099}http://bit.ly/RZCLIENTE");
case 9:SendClientMessageToAll(COLOR_ARTICULO, "[+]{FFFFFF} �Alguien est� haciendo trampa? Reportelo con el comando /reportar");
...
case 15:SendClientMessageToAll(COLOR_ARTICULO, "[+]{FFFFFF} �Muy D�ficil? Puede obtener una membresia VIP con puntos o pago mensual de paypal {FF0099}/rvip");
...
case 20:SendClientMessageToAll(COLOR_ARTICULO, "[+]{FFFFFF} �RECOLECTA! TODAS LAS ARMAS ESTAN PERMITIDAS, �S�! TODAS!");
case 21:SendClientMessageToAll(COLOR_ARTICULO, "[+]{FFFFFF} �S� UN ZOMBIE! Los jugadores VIP tienen la posibilidad de ser zombies con diversas habilidades!");
case 22:SendClientMessageToAll(COLOR_ARTICULO, "[+]{FFFFFF} Visita la p�gina web para adquirir el cliente: {F500F5}www.Revolucion-Zombie.com");
case 23:SendClientMessageToAll(COLOR_ARTICULO, "[+]{FFFFFF} �Sabias Qu�.. El Cliente del servidor (RZClient) otorga armas adicionales cada vez que reapareces?");
case 24:SendClientMessageToAll(COLOR_ARTICULO, "[+]{FFFFFF} �Sabias Qu�.. El Cliente del servidor (RZClient) bonif�ca cada kill que haces?");
}
for(new npcid = 0; npcid < GetMaxPlayers(); npcid++)
{
	...
	if(GetPickupStatus(pickupid))
	{
	    if(IsPlayerOnMission(playerid) && GetMissionType(playerid) == 3)
	    {
			if(GetPickupModel(pickupid) == GetMissionObjType(playerid))
			{
				...
				{
					PlayerPlaySound(playerid, 1057, 0.0, 0.0, 0.0);
					SendClientMessage(playerid,red,"]<!>[ {ffffff}T� mision est� completa, dirigete con cualquier cientifico para recibir tu recompensa.");
					}else{
		...
		if(GetPickupModel(pickupid) != 1582)
		{
			...
			SendFormatMessage(playerid,red,"]<!>[ {ffffff}Has encontrado %s! Recibiste $%d monedas, puedes conseguir m�s trabajando en una misi�n.", gpickupName, moneyadd);}
			else {
...
stock UpdateCoreRoundStatus()
{
	...
	if(HumanCore[CURRENT_ESCAPES] >= HumanCore[MAX_ESCAPES])
	{
		...
		SendClientMessageToAll(red,">] RZ [< {ffffff}Mision fallida, suerte para la pr�xima.");
		SendClientMessageToAll(red,"********");
	}
                ...
                if(PlayerInfo[HumanID][DRONE_ID] == playerid)
                {
              		if(IsHumanStreamedInFromEx(playerid,HumanID, x,y,z, hx,hy,hz, NPC_INFO[playerid][DRONE_MAX_OWNER_DISTANCE]))
              		{
						...
						if(PlayerInfo[HumanID][InCORE] == 1 && !IsPlayerAdmin(HumanID)){
						    ...
						    //SendFormatMessageToAll(0x0FFFF66FF, "<!> %s [%d] {ffffff}  dej� de protgeger a: {FF33CC}%s",PlayerName2(playerid), playerid, PlayerName2(HumanID) );
              		    ...
              		    //SendFormatMessageToAll(0x0FFFF66FF, "<!> %s [%d] {ffffff} est� protegiendo a: {FF33CC}%s",PlayerName2(playerid), playerid, PlayerName2(HumanID) );
		            ...
		            else if (HumanID != INVALID_PLAYER_ID)
					{
					    ...
					    SendClientMessage(HumanID,red,">] DroneSystem [< {ffffff}Haz perdido t� drone, te alejaste demasiado de �l.");
					    //SendFormatMessageToAll(0x0FFFF66FF, "<!> %s [%d] {ffffff}  dej� de protgeger a: {FF33CC}%s",PlayerName2(playerid), playerid, PlayerName2(HumanID) );
						PlayerInfo[HumanID][DRONE_ID] = INVALID_PLAYER_ID;//unset drone for playerid.
				 		NPC_INFO[playerid][LastFinded] = INVALID_PLAYER_ID;
				 		NPC_INFO[playerid][Attacking] = INVALID_PLAYER_ID;
...
PUBLIC:OnCoreZombieUpdate(playerid, victimID)
{
	if(NPC_INFO[playerid][InCORE] == 1)
	{
		...
		MoveNPC(playerid,random(5) - random(7) +x, random(7) - random(7) +y, z, HumanCore[ZOMBIES_MOVE_TYPE], HumanCore[ZOMBIES_MOVE_SPEED]);//�ACA!!
		*/
		return 1;
	}
	return 0;
...
*/
	...
	if(NPC_INFO[playerid][LastFinded] != INVALID_PLAYER_ID||NPC_INFO[playerid][Attacking] != INVALID_PLAYER_ID)
	{
		if((rztickcount() - ZS_Tick[playerid]) > 20 && GetTotalZombiesInPlayer(victimID) <= 20)//rugir si pasaron m�s de 20seg y son menos de 20 zombis
		{
		...
		else if((rztickcount() - ZS_Tick[playerid]) > 20 && ObtenerBOTCercano(victimID) == playerid)//rugir si pasaron m�s de 20seg y si es el zombi mas cercano
		{
...
public OnGameModeInit()
{
	...
	*/
	...
	CreateServerActor(1, 70, 856.3638,-2119.7563,20.0042,359.4094);//Cient�fico Muelle LS
	...
	CreateServerActor(3, 70, 2557.7673, 2783.7126, 13.5202, 87.1632);//Cient�fico refugio cerca del tren LV
	...
	CreateServerActor(5, 70, 2557.8545, 2770.7224, 13.5202, 88.5772);//Cient�fico refugio cerca del tren LV
	...
	*/
	...
	//Timer_Transmi = SetTimer("Transmi",480000,1);//8 min xd
   	SendRconCommand("mapname Espa�ol - Zombie");
	...
	CreateSafeZone(1, -1930.0000457763672,-1708.0000915527344,-1799.0000457763672,-1592.0000915527344);//Monta�a Gigante
