...
void Session::handleRead(const boost::system::error_code &error, std::size_t bytesTransferred)
{
	...
	else
	{
		if (connected)
		{
			logprintf("*** RZC: %s (ID: %d) has disconnected", playerName.c_str(), playerID);
...
void Session::parseBuffer(const std::string &buffer)
{
	...
	if (commandTokens.size() < 2 || commandTokens.size() > 5)
	{
		return;
	}
	...
	case fileChecked:
	{
		return performFileChecked();
	}
	case filesGET:
	{
		return performFilesGET();
	}
	}
}
...
void Session::performAuthenticate()
{
	...
	{
		//sendAsync(boost::str(boost::format("%1%\t%2%\n") % Server::Message % (boost::str(boost::format("The RZClient Owner has deprecated this version %1%") % CLIENT_VERSION))));
		//logprintf("*** RZClient: Cliente %s Rechazado version invalida", p->second.name.front().c_str());
		stopAsync();
		return;
	}
	for (std::map<int, Data::Player>::iterator p = core->players.begin(); p != core->players.end(); ++p)
	{
		if (boost::algorithm::equals(p->second.name.front(), commandTokens.at(1)))
		{
			...
			/*if (RZC_StopSync[p->first] == true) //desincronizar jugador
			{
				//sendAsync(boost::str(boost::format("%1%\t%2%\n") % Server::Message % (boost::str(boost::format("The RZClient Owner has deprecated this version %1%") % CLIENT_VERSION))));
				logprintf("*** RZClient: Player %s Desincronizador por RZC_StopSync", p->second.name.front().c_str());
				stopAsync();
				return;
			}*/
			if (c != core->getServer()->clients.end())
			...
			if (boost::algorithm::equals(sessionAddress, p->second.address))
			{
				...
				logprintf("*** RZClient: %s (ID: %d) se ha conectado", p->second.name.front().c_str(), p->first);
...
void Session::performSequence()
{
	...
	for (std::vector<int>::iterator a = s->second.audioIDs.begin(); a != s->second.audioIDs.end(); ++a)
	{
		...
		{
			int secundxd = t->second + 32;
			if (t->second < secundxd)
			{
				try
				{
...
void Session::performFilesGET()
{
	if (!connected || commandTokens.size() != 5)
	{
		//logprintf("*** RZC Plugin: commandtokens: %d", commandTokens.size());
		return;
	}
	int handleID = 0;
	try
	...
	{
		//logprintf("*** RZC Plugin: Handlexical Error");
		return;
	}
	//logprintf("*** RZC Plugin: playerid omg: %d returnto: %d", playerID, handleID);
	boost::algorithm::replace_all(commandTokens.at(3), "%", "");
	boost::algorithm::replace_all(commandTokens.at(4), "%", "");
	Data::Message message;
	message.array.push_back(Data::OnfilesGET);
	message.array.push_back(playerID);
	message.array.push_back(handleID);
	message.buffer.push_back(commandTokens.at(3));
	message.buffer.push_back(commandTokens.at(4));
	core->messages.push(message);
	
}

void Session::performFileChecked()
{
	if (!connected || commandTokens.size() != 3)
	{
		return;
	...
	boost::algorithm::replace_all(commandTokens.at(2), "%", "");
		Data::Message message;
		message.array.push_back(Data::OnFileChecked);
		message.array.push_back(handleID);
		message.array.push_back(playerID);
		message.buffer.push_back(commandTokens.at(2));
		core->messages.push(message);
