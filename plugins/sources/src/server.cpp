...
void Server::startAsync()
{
	...
	logprintf("*** RZC Plugin: Servidor TCP iniciado en el puerto %d", port);
	io_service.reset();
	boost::system::error_code error;
	io_service.run(error);
	logprintf("*** RZC Plugin: Servidor TCP detenido en el puerto %d", port);
}
...
bool Server::createAcceptor(unsigned short port)
{
	...
	if (error)
	{
		logprintf("*** RZC Plugin: Error opening acceptor: %s", error.message().c_str());
		acceptor.reset();
		return false;
	}
	...
	if (error)
	{
		logprintf("*** RZC Plugin: Error setting option for acceptor: %s", error.message().c_str());
		acceptor.reset();
		return false;
	}
	...
	if (error)
	{
		logprintf("*** RZC Plugin: Error binding endpoint for acceptor: %s", error.message().c_str());
		acceptor.reset();
		return false;
	}
	...
	if (error)
	{
		logprintf("*** RZC Plugin: Error listening for acceptor: %s", error.message().c_str());
		acceptor.reset();
		return false;
	}
	startAccept();
