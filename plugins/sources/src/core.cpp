...
bool Core::setPack(const std::string &name, bool transferable, bool automated)
{
	...
	if (error < 0)
	{
		logprintf("*** RZClient_SetPack: Error opening audio.ini");
		return false;
	}
	bool result = false;
	...
	if (!result)
	{
		logprintf("*** RZClient_SetPack: Audio pack \"%s\" not found in audio.ini", name.c_str());
		return false;
	}
	...
	if (sectionData)
	{
		...
		for (CSimpleIniA::TKeyVal::const_iterator k = sectionData->begin(); k != sectionData->end(); ++k)
		{
			...
			{
				logprintf("*** RZClient_SetPack: Error mapping \"%s\" (invalid audio ID: \"%s\")", value, item);
				continue;
			}
			std::map<int, Data::File>::iterator f = files.find(fileID);
			if (f != files.end())
			{
				logprintf("*** RZClient_SetPack: Error mapping \"%s\" (audio ID %s has already been mapped)", value, item);
				continue;
			}
			if (boost::algorithm::istarts_with(buffer, "http://"))
			{
				...
				{
					logprintf("*** RZClient_SetPack: Error mapping \"%s\" (URL must be 256 characters or less)", buffer.c_str());
					continue;
				}
				...
				if (name.length() > 128)
				{
					logprintf("*** RZClient_SetPack: Error mapping \"%s\" (file name must be 128 characters or less)", buffer.c_str());
					continue;
				}
				if (!transferable)
				{
			...
			else
			{
				...
				if (transferable)
				{
					...
					if (!fileCRC)
					{
						logprintf("*** RZClient_SetPack: Error opening \"%s\" for CRC check", fileLocation.c_str());
						fileCRC.close();
						continue;
					}
					...
					if (!fileIn)
					{
						logprintf("*** RZClient_SetPack: Error opening \"%s\" for file size check", fileLocation.c_str());
						fileIn.close();
						continue;
					}
	...
	logprintf("*** RZClient_SetPack: Audio pack set to \"%s\" (%s and %s)", name.c_str(), transferable ? "transferable" : "not transferable", automated ? "automated" : "not automated");
